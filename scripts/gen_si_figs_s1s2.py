...
# ===== Figure S1: Side-by-side bar chart =====
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 8))
fig.suptitle("Figure S1. Leave-One-PFAS-Out Cross-Validation Comparison", fontsize=13, fontweight='bold')

for ax, results, title, color_prefix in [
    (ax1, combined_sorted, '(a) RDKit-only model', 'RDKit'),
    (ax2, [r for c_name in names_sorted for r in [next(x for x in rdkit_only if x['pfas'] == c_name)]], '(b) Combined model', 'Comb')
]:
    # Re-sort for each panel
    ...
    for v, n in zip(r2s, n_tests):
        if v > 0.5: colors.append('#4daf4a')
        elif v > 0: colors.append('#ffd700')
        elif n < 5: colors.append('#b0b0b0')
        else: colors.append('#e41a1c')

    bars = ax.barh(range(len(names)), r2s, color=colors, alpha=0.8, edgecolor='gray', linewidth=0.3)
...
# Need to re-run LOO to get per-point predictions. Load the feature matrix and PFAS properties.
...
for test_pfas in pfas_list:
    ...
    X_train, y_train = extract_matrix(train_rows)
    X_test, y_test = extract_matrix(test_rows)

    # RDKit-only model: remove soil features
    soil_cols = {'Corg_%', 'foc', 'pH', 'Sand', 'Silt', 'Clay', 'CEC', 'Fe_g_kg', 'Al_g_kg'}
    soil_indices = [i for i, c in enumerate(desc_cols) if c in soil_cols]
    rdkit_indices = [i for i in range(len(desc_cols)) if i not in soil_indices]

    # Combined model
    model_combined = xgb.XGBRegressor(n_estimators=500, max_depth=8, learning_rate=0.05,
    ...
    # RDKit-only model
    if rdkit_indices:
        model_rdkit = xgb.XGBRegressor(n_estimators=500, max_depth=8, learning_rate=0.05,
                                        subsample=0.8, colsample_bytree=0.8, random_state=42, n_jobs=1)
...
fig.suptitle("Figure S2. Leave-One-PFAS-Out Predicted vs. Observed log K$_{d}$", fontsize=13, fontweight='bold')
